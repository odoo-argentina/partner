...
def _genera_detalle_factura (lineafact,cr, doc,lote_comprobantes, comprobante, detalle, nro_linea, temp_buffer):
	...
	valor = doc.createTextNode (str(lineafact.price_unit))
	...
	valor = doc.createTextNode (str(lineafact.price_subtotal))
...
#	GENERA EL RESUMEN DE CADA FACTURA
	...
	valor = doc.createTextNode (str("%.2f" % invoice.amount_untaxed))
	...
	valor = doc.createTextNode (str("%.2f" % invoice.amount_tax))
	...
	valor = doc.createTextNode (str("%.2f" % invoice.amount_total))
	...
	codigo_moneda.appendChild (valor)
        ...
        valor = doc.createTextNode (str(invoice.comment))
	if not valor :
		valor = ""
